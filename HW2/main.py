...
class AugmentedTestDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # image = v2.ToTensor()(image).to(torch.float32).to(device)
        # image = self.aug(image=image.permute(1, 2, 0).cpu().numpy())['image']  # convert to the image format
        # image = torch.tensor(image).permute(2, 0, 1).to(torch.float32).to(device)  # convert to numpy format
        # return image, label
...
augmented_train_set = AugmentedTestDataset(train_set)
...
for i in range(len(train_set)):
    image, _ = train_set[i]
    augmented_image, _ = augmented_train_set[i]
